...
def get_RadarState_from_vision(lead_msg: capnp._DynamicStructReader, v_ego: float, model_v_ego: float, lead_index: int):
...
def get_lead(v_ego: float, ready: bool, tracks: Dict[int, Track], lead_msg: capnp._DynamicStructReader,
             model_v_ego: float, lead_index: int, low_speed_override: bool = True) -> Dict[str, Any]:
  ...
  if track is not None:
    ...
    lead_dict = get_RadarState_from_vision(lead_msg, v_ego, model_v_ego, lead_index)
...
class RadarD:
  ...
  def update(self, sm: messaging.SubMaster, rr: Optional[car.RadarData]):
    ...
    if len(leads_v3) > 1:
      self.radar_state.leadOne = get_lead(self.v_ego, self.ready, self.tracks, leads_v3[0], model_v_ego, 0, low_speed_override=True)
      self.radar_state.leadTwo = get_lead(self.v_ego, self.ready, self.tracks, leads_v3[1], model_v_ego, 1, low_speed_override=False)
