...
def get_lead(v_ego, ready, clusters, lead_msg, model_v_ego, lead_index, low_speed_override=True):
  ...
  if cluster is not None:
    ...
    lead_dict = Cluster().get_RadarState_from_vision(lead_msg, v_ego, model_v_ego, lead_index)
...
class RadarD():
  ...
  def update(self, sm, rr):
    ...
    if len(leads_v3) > 1:
      radarState.leadOne = get_lead(self.v_ego, self.ready, clusters, leads_v3[0], model_v_ego, 0, low_speed_override=True)
      radarState.leadTwo = get_lead(self.v_ego, self.ready, clusters, leads_v3[1], model_v_ego, 1, low_speed_override=False)
    return dat
